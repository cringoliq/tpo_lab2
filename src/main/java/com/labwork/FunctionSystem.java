...
public class FunctionSystem implements FunctionInterface {
    ...
    public double calculate(double x, double delta) {
        if(x<=0){
            return (Math.pow(((Math.pow((Math.pow(((((Math.pow((((((Math.pow((tan.calculate(x, delta) + cos.calculate(x,delta)), 3)) + cot.calculate(x, delta)) - (tan.calculate(x, delta) - csc.calculate(x, delta)))
                    - sec.calculate(x, delta)) - csc.calculate(x, delta)), 3)) + (csc.calculate(x, delta) / Math.pow((cot.calculate(x, delta)), 2))) -
